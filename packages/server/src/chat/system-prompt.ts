...
function formatVoiceModeFragment(): string {
  return [
    ...
    `Wrap EACH natural spoken chunk in <chunk>...</chunk> tags. A chunk is a segment you'd say between breaths — typically one to three sentences, or one paragraph. The client synthesises and plays each chunk in order as it arrives, so smaller well-formed chunks give faster first-audio and smoother pacing.`,
    ``,
    `Rules for <chunk>:`,
    `  - Every spoken portion of your reply MUST be inside a <chunk>...</chunk>.`,
    `  - Text outside <chunk> tags is treated as visual-only annotation and skipped by TTS.`,
    `  - Open and close each chunk on the same output pass so the client can start speaking as soon as it arrives.`,
    `  - Don't nest chunks. <silent>...</silent> can appear INSIDE a chunk to skip technical tokens.`,
    `  - Aim for chunks of 1-3 sentences (roughly 20-80 characters of Chinese, 10-25 words of English). Very short one-word chunks waste round trips; very long chunks delay first audio.`,
    ``,
    `<silent>...</silent> escape hatch: when a technical token MUST appear on screen but reading it aloud would sound wrong (hex hashes, file paths, long URLs, phone numbers), wrap that token in <silent>...</silent>. The visible bubble still shows the wrapped content; TTS just skips it. <silent> lives INSIDE <chunk>.`,
    ``,
    `Example (fictional short reply):`,
    `  <chunk>已经把修复合进去了。</chunk>`,
    `  <chunk>你看下最新的 commit 就知道了，小尾巴是 <silent>7f3a8b2c</silent>。</chunk>`,
    `  <chunk>如果还有问题告诉我。</chunk>`,
    ``,
    `Reminder: don't summarise — the user hears the chunk content verbatim.`,
  ].join("\n");
}
