...
function formatVoiceModeFragment(): string {
  return [
    ...
    `**Wrap EVERY sentence of your reply in <chunk>...</chunk> tags.** No exceptions. This applies to short replies AND long stories, opening lines AND every subsequent sentence.`,
    ``,
    `The client synthesises and plays each chunk in order as it arrives. If a sentence isn't inside a chunk, it will NOT be spoken — the user will read it on screen but hear silence.`,
    ``,
    `Rules for <chunk>:`,
    `  - EVERY sentence gets its own <chunk>...</chunk>. Not just the opener.`,
    `  - No unwrapped prose. If you write a paragraph of 5 sentences, that's 5 chunks in a row.`,
    `  - Aim for 1-2 sentences per chunk (roughly 15-60 CJK chars, 8-20 EN words).`,
    `  - Open and close each chunk on the same output pass.`,
    `  - Don't nest chunks. <silent>...</silent> can appear INSIDE a chunk to skip technical tokens.`,
    ``,
    `<silent>...</silent> escape hatch: for hex hashes, file paths, long URLs, phone numbers — wrap so TTS skips them. Lives INSIDE <chunk>.`,
    ``,
    `Example — short reply (3 sentences → 3 chunks):`,
    `  <chunk>已经把修复合进去了。</chunk>`,
    `  <chunk>你看下最新的 commit 就知道了，小尾巴是 <silent>7f3a8b2c</silent>。</chunk>`,
    `  <chunk>如果还有问题告诉我。</chunk>`,
    ``,
    `Example — long reply / story (still every sentence in a chunk):`,
    `  <chunk>好，再来一段。</chunk>`,
    `  <chunk>从前有个小镇。</chunk>`,
    `  <chunk>镇上住着一个叫阿云的小女孩。</chunk>`,
    `  <chunk>阿云每天早上到海边看日出，一看就是五年。</chunk>`,
    `  <chunk>五年后的一个早上，她没来。</chunk>`,
    `  <chunk>那天的日出也没来。</chunk>`,
    `  <chunk>后来投诉发现，两件事没有关系。</chunk>`,
    ``,
    `Reminder: don't summarise — the user hears the chunk content verbatim. If content isn't in a chunk, they don't hear it at all.`,
  ].join("\n");
}
