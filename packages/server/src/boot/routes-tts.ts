...
export function mountTtsRoutes(app: Express) {
  ...
  app.get("/api/tts/status", async (req: Request, res: Response) => {
    const ttsUrl = process.env.TTS_URL || DEFAULT_COSY_URL;
    let ttsReachable: boolean | null = null;
    let customVoices: string[] = [];
    let presetVoices: string[] = [];
    let defaultVoice = "";
    ...
    if (shouldProbe) {
      try {
        ...
        if (probe.ok) {
          const health = (await probe.json()) as Record<string, unknown>;
          customVoices = Array.isArray(health.custom_voices)
            ? health.custom_voices
            : [];
          presetVoices = Array.isArray(health.preset_voices)
            ? health.preset_voices
            : [];
          defaultVoice =
            typeof health.default_voice === "string"
              ? health.default_voice
              : "";
        }
      } catch {
        ttsReachable = false;
    ...
    res.json({
      ...
      cosyvoiceUrl: ttsUrl,
      cosyvoiceReachable: ttsReachable,
      // Voice info from /health
      customVoices,
      presetVoices,
      defaultVoice,
    });
