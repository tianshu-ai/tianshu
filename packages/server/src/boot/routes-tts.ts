...
export function mountTtsRoutes(app: Express) {
  app.post("/api/tts", async (req: Request, res: Response) => {
    ...
    } else if (TTS_PROVIDER === "cosyvoice") {
      await handleCosyvoice(res, text, voice);
    } else {
      console.warn(`[tts] unknown TTS_PROVIDER: ${TTS_PROVIDER}`);
      res.status(500).json({
        error: `unknown TTS_PROVIDER: ${TTS_PROVIDER}`,
        hint: "Set TTS_PROVIDER=edge or TTS_PROVIDER=cosyvoice",
      });
    }
  });

  console.log(`[tts] mounted /api/tts provider=${TTS_PROVIDER}`);
}
